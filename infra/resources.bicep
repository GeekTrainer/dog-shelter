...
var clientAppSettingsArray = filter(array(clientDefinition.settings), i => i.name != '')
var clientSecrets = map(filter(clientAppSettingsArray, i => i.?secret != null), i => {
  name: i.name
  value: i.value
  secretRef: i.?secretRef ?? take(replace(replace(toLower(i.name), '_', '-'), '.', '-'), 32)
})
var clientEnv = map(filter(clientAppSettingsArray, i => i.?secret == null), i => {
  name: i.name
  value: i.value
})
